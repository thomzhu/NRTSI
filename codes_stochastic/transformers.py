...
class Encoder(nn.Module):
    ...
    def __init__(
            ...
            d_model, d_inner, d_data, confidence, n_mix=1, dropout=0.1, use_layer_norm=0, use_gap_encoding=0, adapter=0, use_mask=0):
        ...
        if confidence:
            if n_mix > 1:
                self.output_proj = nn.Linear(d_model, 2*n_mix*d_data + n_mix)
            else:
                self.output_proj = nn.Linear(d_model, 2*d_data)
        else:
